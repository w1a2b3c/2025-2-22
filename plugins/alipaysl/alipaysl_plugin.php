...
class alipaysl_plugin
{
	static public $info = [
		...
		'inputs' => [ //支付插件要求传入的参数以及参数显示名称，可选的有appid,appkey,appsecret,appurl,appmchid
			...
			'apptype' => [
				'name' => '<script>function set_apptype(){ document.querySelector(\'input[name="config[apptype]"]\').value=\'[apptype]\';}</script>支付模式（<a onclick="set_apptype()">主通道填写：[apptype]</a>）<br><span style="font-size:13px;line-height:27px;font-family: \'SimSun\', sans-serif;">1.网站支付&ensp;&emsp;&emsp;&emsp;2.WAP支付&ensp;&emsp;&emsp;3.当面付扫码&ensp;&emsp;4.当面付JS<br>5.预授权支付&ensp;&emsp;&emsp;6.APP支付&ensp;&emsp;&emsp;7.JSAPI支付&ensp;&emsp;&ensp;8.订单码支付</span>',
				'type' => 'input',
				'note' => '',
			],
			'appmchid' => [
				'name' => '<script>function set_submchid(){ document.querySelector(\'input[name="config[appmchid]"]\').value=\'[submchid]\';}</script>商户授权token（<a onclick="set_submchid()">主通道填写：[submchid]</a>&emsp;<a href="alipaysl_auth.php" target="_blank">子通道获取</a>）',
				'type' => 'input',
				'note' => '',
			],
		],
